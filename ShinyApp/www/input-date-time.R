...
dateTimeInput <- function(inputId, label, value = NULL, min = NULL, max = NULL,
                          format = "YYYY-MM-DD HH:mm", startview = "month", weekstart = 0, language = "en",
                          width = NULL) {
        ...
        tagList(
                singleton(
                        tags$head(
                                includeScript("js/moment.js"),
                                includeScript("js/bootstrap-datetimepicker.js"),
                                includeScript("js/date-time-input-bindings.js"),
                                includeCSS("css/bootstrap-datetimepicker.css")
                                #includeCSS("css/bootstrap-combined.min.css")
                        )
                ),
                #         attachDependencies(
                tags$div(id = inputId,
                         class = "date-time-input form-group shiny-input-container",
                         ...
                         tags$input(type = "text",
                                    ...
                                    `data-date-time-language` = language,
                                    `data-date-time-weekstart` = weekstart,
                                    `data-date-time-format` = format,
                                    `data-date-time-start-view` = startview,
                                    `data-min-date-time` = min,
                                    `data-max-date-time` = max,
                                    `data-initial-date-time` = value
                         )
                )
...
registerInputHandler("shiny.dateTimeInput", function(data, ...) {
        if (is.null(data))
                NULL
        else
                data
